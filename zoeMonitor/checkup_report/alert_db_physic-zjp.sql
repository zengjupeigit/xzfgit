...
BEGIN
	...
	if lv_datas-lv_datafiles<10 then 
	...
	else if lv_datas-lv_datafiles<50 and lv_datas-lv_datafiles>10 then
		DBMS_OUTPUT.PUT_LINE('<table WIDTH=600 BORDER=1>');
	...
	if lv_ctlfiles <2 then
		DBMS_OUTPUT.PUT_LINE('<table WIDTH=600 BORDER=1>');
		DBMS_OUTPUT.PUT_LINE('<td>严重警告！当前控制文件个数：'||lv_ctlfiles||'，请及时处理</td>');
		DBMS_OUTPUT.PUT_LINE('</table> ');
	end if;
END; 
/